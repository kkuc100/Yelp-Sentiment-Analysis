...
class Model(torch.nn.Module):
    def __init__(
        ...
        token: str = None,
        _num_labels: int = None,
        unfreeze_layers: str = None,
        device: str = None,
        dropout: float = 0,
        activation: str = "ReLU",
        hidden_size: int = 0,
        layer_sizes: str = None,


    ):
        super().__init__()
        self._num_labels = _num_labels
        ...
        self.token = token
        self.layer_sizes = layer_sizes
        self.layer_builder = LayerBuilder(self.hidden_size, self.layer_sizes, self._num_labels, self.activation, self.dropout)


        if "t5" in self._model_name:
            config = T5Config.from_pretrained(
                self._model_name, output_hidden_states=True, num_labels=self._num_labels
            )
            self.pretrained = T5ForConditionalGeneration.from_pretrained(
                self._model_name, config=config, token=token
            ).to(device)
        else:
            self.pretrained = AutoModelForSequenceClassification.from_pretrained(
                self._model_name, token=token
            ).to(device)

        self.freeze_params(self.unfreeze_layers)

        self.additional_layers = self.layer_builder.build().to(device)
